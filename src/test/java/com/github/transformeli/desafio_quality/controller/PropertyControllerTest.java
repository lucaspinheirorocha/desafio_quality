...
class PropertyControllerTest {
    ...
    void propBiggestRoom_returnSucessfully_whenWithPropertyContent() {
    ...
    @Test
    @DisplayName("Return falied max room by without property contents")
    void propBiggestRoom_returnFalied_whenWithOutPropertyContent() {
        BDDMockito.when(service.propBiggestRoom(ArgumentMatchers.any(Property.class))).thenReturn(TestUtilsProperty2.propBiggestNoRoom());
    ...
    public void createProperty_returnSuccessfully_whenWithPropertyContent() {
        ...
        Property newProperty = TestUtilsProperty2.getNewProperty();
    ...
    @Test
    @DisplayName("Return falied created a new property")
    public void createProperty_returnFalied_whenPropertyNoContent() {
    ...
    public void updateProperty_returnSuccessfully_whenWithPropertyContent() {
        BDDMockito.when(service.updateProperty(ArgumentMatchers.anyString(), ArgumentMatchers.any(Property.class))).thenReturn((TestUtilsProperty.getNewProperty()));

        String name = "Casa da Lari";
        Property newProperty = TestUtilsProperty.getNewProperty();
        ResponseEntity<Property> response = controller.updateNProperty(newProperty,name);
        Assertions.assertThat(response.getBody()).isInstanceOf(Property.class);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().getRooms().size())
                .isNotNull()
                .isPositive();
        verify(service, atLeastOnce()).updateProperty(name, newProperty);
    }

    @Test
    @DisplayName("Return falied updated property")
    public void updateProperty_returnFalied_whenWithoutPropertyContent() {
        BDDMockito.when(service.updateProperty(ArgumentMatchers.anyString(), ArgumentMatchers.any(Property.class))).thenReturn((TestUtilsProperty.getNewProperty()));
    ...
    void deleteProperty_returnSucessfully_whenWithPropertyContent() {
    ...
    void deleteProperty_returnFalied_whenWithoutPropertyContent() {
        BDDMockito.when(service.deleteProperty(ArgumentMatchers.anyString())).thenReturn(false);
